...
Research prototype: iPhone app that records observable gaze, eye/brow blend-shape,
head-pose and UI interaction signals via ARKit, and exports them as a normalized
JSON event stream for analysis in Python.

**Read first:** `docs/INTERACTION_FINGERPRINT_XCODE_SETUP.md` (milestones, signals to
record, definition of done) and the project skills in `.claude/skills/`.

## Project rules

- Capture observable signals. Never label them as emotions or intent in code or data.
- Keep V0 small: ARKit → raw signals → local JSON export. No agents, no generative UI yet.
- ARKit face tracking needs a physical iPhone. Simulator builds only verify compilation.
- Feature code goes in `InteractionFingerprintPackage/Sources/InteractionFingerprintFeature/`
  under `Views/`, `Tracking/`, `Fingerprint/`, `Models/`, `Storage/`. The app target is a shell.
- Never commit anything under `Data/` (participant sessions), video, keys, or signing files.
- Build settings live in `Config/*.xcconfig`. The camera usage string is set there.
...
- **Entitlements File**: `Config/InteractionFingerprint.entitlements` contains all app capabilities
...
To add capabilities to your app, edit `Config/InteractionFingerprint.entitlements`:
...
// In your app
@main
struct InteractionFingerprintApp: App {
    var body: some Scene {
        WindowGroup {
            ContentView()
